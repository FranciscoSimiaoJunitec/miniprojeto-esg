...
const MapText: React.FC = () => {
  ...
  return (
    <div id="map-section" className="min-h-screen flex flex-col md:flex-row items-start justify-between bg-gradient-to-b from-[#3e7ab9] to-[#053581] p-8 text-white relative">
      {/* Text Section */}
      ...
      <div className="w-full md:w-1/2 relative" style={{ minHeight: '700px' }}>
        <SlideInAnimation direction="right" className="absolute z-30 top-[55%] left-[-15%] transform translate-x-[-55%] translate-y-[-45%] scale-[0.95] w-[70%]">
          <MadeiraMap tooltipId="madeira-tooltip" selectedRegion={selectedRegion} setSelectedRegion={setSelectedRegion} />
        </SlideInAnimation>
        <SlideInAnimation direction="right" className="absolute inset-0 z-10 right-[-20%] transform translate-x-[-12.2%] scale-[1]">
          <PortugalMap tooltipId="portugal-tooltip" selectedRegion={selectedRegion} setSelectedRegion={setSelectedRegion} />
        </SlideInAnimation>
      </div>
